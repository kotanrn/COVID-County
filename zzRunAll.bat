@echo off

C:\python27\python.exe County.py 15 "Bell, Texas, US"
C:\python27\python.exe County.py 15 "Coryell, Texas, US"
C:\python27\python.exe County.py 15 "Sweden"
C:\python27\python.exe County.py 15 "Bulgaria"
C:\python27\python.exe County.py 15 "Hungary"
C:\python27\python.exe County.py 15 "Latvia"
C:\python27\python.exe County.py 15 "Lithuania"
C:\python27\python.exe County.py 15 "Poland"
C:\python27\python.exe County.py 15 "Romania"

C:\python27\python.exe US_time.py 15
C:\python27\python.exe Germany_time.py 15

C:\python27\python.exe States.py

pause
